...
| **8.2** | (26/09): **o tamanduá de olhos de caracol** — "um tamanduá com olhos de caracol que lambe cogumelos". Pelo violeta com a faixa diagonal escura de borda creme, focinho longo, cauda peluda de pontas turquesa, hastes luminosas com olhos âmbar. Anda devagar até um cogumelo (pela lateral, contornando quem está no centro), ergue o focinho e lambe o chapéu três vezes com a língua rosa — o cogumelo muda de cor; tocado, recolhe os olhos como caracol | `index` e `v6.html` | `v8.2` |
| **8.3** | (26/09): **o espaço de cada corpo** — "os objetos não podem se penetrar… espaçamento… de acordo com o mapeamento do local… próximos, campo médio, longe, muito longe… não precisa deixar tudo tão separado, mas que respeitem a área do objeto". Cogumelos e tufos de folhas se espalham antes de nascer (`separarOsCorpos`: faixas perto 1,3–1,7 m e meio 1,9–2,6 m, 1 m livre em volta de quem começa, 30 cm das paredes lidas, folgas de 3 a 6 cm além da forma); o que anda pergunta a cada quadro se está dentro de tronco (medido na altura dele), chapéu, pedra ou tufo, e sai (`afastar`) — tartaruga, tamanduá (corpo, cabeça e cauda), coruja (só o poleiro de destino é porta), abelha, vagalumes, seres de luz nos gigantes do mar, planetas soltos entre si, arcos sem invadir o vizinho. A folhagem padronada toda **no chão** (`TUFOS_CHAO`: quatro ao pé da mãe, ao pé das três árvores mais perto, três soltos no campo médio; tamanhos de 0,5 a 1,3; leque para fora do tronco). Plantas nas paredes até nove, duas por parede larga, a 55 %. **O céu desce mais** (`ini/fim` de todas as `CENAS`; com paredes lidas, até −0,42). As quatro faixas do Suno em `assets/audio/suno/`. Bancada: `raizes.espaco()`, `raizes.dentro(x,z,y,r)` | `index` e `v6.html` | `v8.3` |
| **8.4** | (26/09, a lista da manhã): **(1) a vitrine só com estrelas** (aos 0:04, era 0:40 — "está dando spoiler") e **o crescimento de volta** (`iniciarObra` zera `crescimento`: a vitrine da 7.9 deixava a floresta já feita ao Iniciar — "perdeu o crescimento das árvores"); **(2) as deusas**: a vermelha 35 % maior, menos vermelha (`satura` 0,72) e no azimute −84; nas três, **manchas de aquarela** nas cores da própria pintura, só no escuro do quadro (`mancha1/2`, com pintas que cintilam), e **as mãos e o rosto se mexem** (`juntas`: três pontos medidos no arquivo, girando e boiando com peso gaussiano — "como se dançassem no espaço"); **(3) o céu do planeta rosa mais alto** (`alto` 0,35: a pintura encolhe para o zênite) com **estrelas menores piscando bem** (44 células, 1 a 3 Hz); **(4) o mar** menos transparente (0,58/0,86), **iridescência leve de volta** (um sexto parado, um terço com o gesto) e cristas mais macias; **(5) a saída do mar**: a água sobe 2,6 m dos 8:20 aos 8:46 (passa pelos olhos aos 8:28) e só então tudo se apaga, até 9:00 (`lamina` enquanto houver água, não só no cenário 3); **(6) os corais vivos** (`CORAIS_VIVOS`, matéria 27): sete corpos opacos e bioluminescentes de cores diferentes no anel de 1,55–2,3 m (coral, conchas, estrela-do-mar, alga enrolada), azulam só 10 % debaixo d'água; tocados, a cor vira película, a onda corre pelo corpo, soa a nota e a água (`TOCOU-UM-CORAL-VIVO`); **(7) o cenário final mais longe**: a íntegra a 52 m (era 40), os arcos de 8 a 40 m (eram 6 a 31), o papel 30 % mais longe e 15 % menor; **(8) o céu final anima**: as seis galáxias giram em volta do próprio centro (`giraAng`, `GALAXIAS_ST`; rígidas no miolo, o fundo se enrola na beira — "aqueles redemoinhos"). O piscar do mar visto por ela não apareceu na bancada (frames consecutivos sem vai-e-volta): ficou a suavização das cristas e a lâmina mais opaca — conferir no Quest. Bancada: `raizes.corais()` | `index` e `v6.html` | `v8.4` |
| **8.5** | (26/09, fim da manhã): **as ondas do fim cheias** — "a forma meio-círculo não pode ser vazada; mais ondas; graduação embaixo": cada onda é um meio-disco inteiro (seis anéis), 6 a 12 por fileira sobrepondo-se como as escamas da pintura, malha da fileira mais longe para a mais perto, e a base dissolve (`hA` na matéria 25); **as plantas grandes do mar mais longe** (anel 3,4–5,0 m, era 2,05–3,4; saem do alcance da mão); **o céu do planeta rosa com a graduação para a RA** dos outros cenários (`ini` 0,10 / `fim` 0,52, começava no horizonte); **as músicas de cada cenário** (`TRILHAS_DA_CENA`, `seguirTrilhasDaCena`): Raízes Mar B nos planetas (`assets/audio/trilha-2-planetas.mp3`, 3:14–6:32) e Raízes Cosmos A no mar (`trilha-3-mar.mp3`, 6:28–8:48), cross-fade de 10 s, sempre do começo, o laço cede a um quinto, abaixam com os gestos; servidas ao lado do arquivo e guardadas pelo `sw.js` para o estande sem rede; **a geometria quebrada da mãe** (vista quando o crescimento voltou na 8.4): o que não tinha brotado ia ao chão e virava fitas — agora fica na altura da frente (`kFrente` no VS_SOLIDA). **Correção 8.5b** (main, sem mover a marca): o `alto` do céu do mar escalava o plano inteiro e lia a pintura fora da área pintada perto do horizonte (faixa escura nas laterais) — agora vale só no alto e volta a 1 no horizonte; a textura gerada pinta a largura inteira; `ini` 0,05 / `fim` 0,56 ("o céu tem que estar bem graduado nas bordas"). Bancada: `raizes.trilhas()` | `index` e `v6.html` | `v8.5` |
| **8.6** | (26/09, tarde): **os planetas mais baixos e um pouco mais perto** — "desce um pouco mais os planetas para eles se movimentarem mais para baixo, e aproxime mais perto das pessoas, mas só um pouco": a órbita de 1,6–1,9 m foi a 1,4–1,65 m, a altura de 1,6–2,0 m a 1,28–1,55 m, o sobe-e-desce de 22 para 30 cm; o rosa, que se pega, ficou. **Estrelas no chão** (`montarEstrelasDoChao`, `bEstrelasChao`): no cosmos, 420 estrelas miúdas deitadas no chão real de 0,5 a 5,5 m, cintilando pelo `progPo`, entrando e saindo com os planetas — pontos esparsos, o chão continua sendo a sala. | `index` e `v6.html` | `v8.6` |
| **8.7** | (26/09, tarde): **dentro da água** — "o mar ainda está piscando… vamos deixar o mar mais alto mesmo, como se estivéssemos dentro da água": `NIVEL_AGUA` 0,80 → 2,35 m, acima de qualquer cabeça; o cenário inteiro é visto de dentro (véu azul do `progMergulho` mais leve, 0,26/0,10; cáusticas no chão real; a superfície como teto de luz). O que se toca continua na altura da mão por `NIVEL_COISAS` = 0,80 (concha, seres de luz, águas-vivas a 1,25–2,15 m, o que boia). O azul dos sólidos, com o olho dentro, é por **distância** (0,30 perto, 0,85 a 7 m), não mais 0,92 por profundidade — o que está a um braço guarda a cor. O toque na lâmina deixa de acontecer (está fora do alcance); a subida final da 8.4 continua, invisível. | `index` e `v6.html` | `v8.7` |
| **8.8** | (26/09, tarde): **as deusas pintadas em código** — "você já retirou as imagens do cenário dos planetas e da cena final da deusa? Tem que refazer com a programação": as três (galáctica, vermelha, íntegra) deixam de ser JPEG e são pintadas ao carregar (`pinturaEmCodigo` + `gerarDeusaGalactica/Vermelha/Integra`: carimbos gaussianos, manchas de aquarela com beira de ruído, traços, fios em passeio aleatório, meio-discos de onda, estrelas de quatro pontas), a 2048×1280, no mesmo lugar do quadro em que a pintura estava (rosto, mãos e elipse continuam valendo). `FIGURAS_EM_CODIGO` no alto do template e no `montar_mr.py`; a borboleta continua PNG. O arquivo caiu de 11,1 para 9,0 MB; `TEX_TOTAL` 14 → 11; a pintura leva menos de 1 s. **Sem mipmap** nessas texturas (1280 não é potência de dois: com mipmap a textura sai preta no WebGL 1). Na mesma rodada: **sem o buraco negro** (`BURACOS_NEGROS = false`), **sem as pedras escuras debaixo d'água** (as onze pedras somem com a água), e **o tamanduá de uma cor só** (o violeta em tudo, com losangos e zigue-zague claros no corpo; só olhos e língua de outra cor) **e destravado** (estava 20 s parado entre cogumelos: se não anda 8 cm em 3 s, vira 90° e troca de cogumelo; o desvio de quem está no meio passou a 1,3 m). **As estrelas do chão** do cosmos começam a 1,5 m (era 0,5). **O espelho alinhado** (`fontes/espelho.html`) ganhou o **QR code do site** (`fontes/qr-visoesfilmes.svg`, sempre no canto, também em tela cheia) e, sem captura ao vivo — antes da primeira e assim que o espelhamento acaba —, **toca a animação da Odara** em laço. | `index` e `v6.html` | `v8.8` |
| **8.9** | **a que está no ar** (26/09, fim da tarde): **o epílogo** — "no final dos 10 minutos coloque 10 segundos a mais indo para só as estrelas, e fica só realidade aumentada para saber que saiu": depois dos 14 s da frase, mais 12 (`fecho` volta, só aqui): o mundo se apaga, o céu recua para o zênite e some, ficam as estrelas (a camada do cenário 1, `estrelasFim`) e **o nome** (`texTitulo`: Raízes Cósmicas · Plinio Hit · Crisia · **Visões Filmes** — "o nosso nome sumiu"; `nomeFim`), e por fim só a sala; a obra para aos 10:26 e espera. **A asa da borboleta**: as faces estavam trocadas (o 88 por cima, o azul por baixo — medido na bancada de cima e de baixo); o azul foi para a camada de normal +y. **A tela real some** (`TAMPAR_A_TELA`): com a tela lida ou cravada, um retângulo do tamanho dela é pintado com o próprio céu (mesmo shader, graduação desligada) — uma janela para o céu no lugar da tela; `raizes.tela.ditar({largura,altura,rumo})` na bancada. **Correção 8.9b** (main): "tire a tela preta da tela real" — `TAMPAR_A_TELA` desligado (na altura dos olhos o céu é quase preto e a tampa lia como retângulo preto), e a tela de tule sem vídeo deixou de ser pintada de escuro: só as ondas do toque aparecem sobre a tela real (`base` no FS_TELA). | `index` e `v6.html` | `v8.9` |
| **8.1** | (26/09): **o cenário 4 e os céus** — sem as árvores invertidas do teto e sem os cogumelos no fim; no lugar, **arcos d'água em semicírculo** (as formas da imagem da deusa) em seis fileiras que convergem para ela, boiando, com borda de ouro e pontos, e véu abaixo da linha dos olhos. **Os céus cósmico e rosa passaram a ser gerados em código** a partir das pinturas (galáxias, rios vermelhos e azuis, ramos dourados, estrelas; mármore rosa com canto lilás, purpurina e rio creme): as quatro pinturas saíram do arquivo, que caiu de 13,1 para 11,1 MB | `index` e `v6.html` | `v8.1` |
| **8.0** | (26/09): **a gigante viva** — "a deusa gigante precisa ficar virada para o usuário, e a cabeça tem que mexer; você tem o tracking do corpo dela". O ser camuflado do céu passou a ser vestido nos ossos da mesma captura da dançarina (a pele por proximidade virou `vestirCorpo`, que serve aos dois corpos), a um quinto da velocidade e 55 vezes maior; o corpo vira devagar para onde a pessoa está e a cabeça inclina para olhá-la. As pernas continuam camufladas como antes. Provada na bancada (floresta e planeta rosa) | `index` e `v6.html` | `v8.0` |
| **7.9** | (25/09, noite): **a vitrine** — "a capa inicial está muito sem graça; uma imagem do cenário se movendo num loop, sem dar spoiler". Atrás do portão a própria obra fica viva, parada aos 0:40 do roteiro (floresta crescida, rios de luz se desenhando; antes dos seres), câmera olhando para cima e girando devagar; o portão virou vinheta translúcida com a caixa de texto sobre fundo escuro. Liga ao terminar de carregar e sempre que o portão volta. Correções da rodada (26/09, em `main`): **a borboleta** — o modelo entrava com +x na esquerda do bicho e o mundo o punha na direita (asa esquerda, com a pintura dela, do lado direito); espelhado no shader. E a inclinação do corpo era um seno solto de ±20°, que rolava o corpo sem curva e lia como "as asas batendo ao contrário"; agora pende na medida da curva. Prompts das trilhas por cenário em `assets/audio/PROMPTS-SUNO.md` | `index` e `v6.html` | `v7.9` |
| **7.8** | (25/09, noite): **de volta aos dez minutos** — "agora tem mais elementos para ver no primeiro cenário". `DURACAO_REAL = 600`; o portão e a Cabine dizem dez | `index` e `v6.html` | `v7.8` |
| **7.7** | (25/09, noite): **as mãos falam a língua de cada cenário** — esporos na Floresta, fios suaves no Cosmos (os das deusas, saindo das mãos), bolhas de ar no Mar com a mão embaixo d'água, pó de estrelas no Papel; **a tartaruga**, que anda devagar pelo chão com o casco de escamas ocre e costura turquesa, recolhe quando tocada; e **a árvore cresce inteiriça**, sem mostrar a geometria por dentro. Depois, na mesma rodada: a tartaruga virou **plantaruga** (casco nas cores e padrões das folhas, com uma plantinha e um cogumelinho em cima); **a mãe a dois passos à direita de quem começa** ("o ambiente se distribui a partir de onde a pessoa começa"); **plantas pontuais nas paredes lidas** e a vida rearrumada depois do escaneamento; as faixas da mãe sempre visíveis e rosa ao aproximar; **o menu da mão pede 5 s de pinça apontando para a frente** e ganhou **Voltar**; e a **qualidade**: escala de desenho 0,7 → 0,8, piso da escala viva 0,75 → 0,82, aquarela e papel menos saturados. Provado na bancada; no Quest ainda não | `index` e `v6.html` | `v7.7` |
| **7.6** | (25/09, noite): **sem montanhas** — "estão num horizonte, e esse horizonte, quando vai para a realidade aumentada, não faz sentido; melhor tirar". `MONTANHAS = false` apaga o horizonte desenhado e o modelo; na linha dos olhos fica só a sala real | `index` e `v6.html` | `v7.6` |
| **7.5** | (25/09, noite): **a coruja-helicóptero** — "animais maiores... uma coruja com olhos grandes, pode ter apenas um olho, e em vez de asas voasse como um helicóptero". Ovo de penas roxas com um olho âmbar só (pisca; a pupila segue a pessoa), rotor de quatro pás e rotor de cauda; pousa no vão da mãe e em cima dos cogumelos, decola na vertical, patrulha a sala como helicóptero (nariz para onde vai, pende nas curvas), vem pairar a um metro e pouco da cara olhando, volta ao poleiro. Tocada, pisca, pia e sobe. Malha própria transformada na CPU (matéria 23). Provada na bancada; no Quest ainda não | `index` e `v6.html` | `v7.5` |
| **7.4** | (25/09, noite): **a vida miúda da Floresta** — "trazer mais vida, está monótono", cor complementar (âmbar, coral, ocre contra o azul), público majoritariamente criança. Vagalumes com três comportamentos (espalhados, enxame em volta da pessoa piscando em onda, revoada em linha), abelha polinizadora de planta em planta, formiguinhas em fila pela mãe, pelas árvores e pelos cogumelos (catando e carregando), folhas padronadas com os padrões das duas referências (folhas pintadas + faixas ancestrais de cerâmica/pintura corporal) que acendem e ondulam com a mão, cogumelos que mudam de cor ao toque, veias fluorescentes e faixas ancestrais na mãe onde a mão passa, tremor e respiração da superfície, pulso do toque mais forte, tocar os seres soa, e a trilha abaixa quando algo soa ("o ambiente está abafando a interação"). Interruptor `VIDA_NA_FLORESTA`. Provado na bancada; no Quest ainda não | `index` e `v6.html` | `v7.4` |
| **7.3** | (25/09, à noite — cinco pedidos numa tarde): **seis minutos** — "faça a experiência ter 6 minutos": a partitura continua escrita em 600 s de roteiro (centenas de instantes literais), mas o relógio do roteiro anda a `RITMO = 600/360` por segundo real (`DURACAO_REAL`); o que é movimento (ondas, seres, física) lê `relogio` e não acelera; a travessia entre cenários e a frase final ficam em segundos reais; a Cabine mostra o tempo real e o de roteiro. **O mar translúcido sem iridescência**: a lâmina a 0,42 a pino (era 0,72), a pele da pintura reduzida à luminância tingida de um azul só (os veios vermelho-verde-azul liam como óleo), e o gesto só clareia a água em vez de acender um arco-íris. **A dançarina longe, mais alta e com os pés no chão**: 3,8 m (era 2,3), 1,55 m na junta da cabeça (≈1,75 no topo; era 1,2), `DANCA_PE = 0`; **e a dança vai e volta** — o tempo bate no último quadro e desce, sem o corte do fim para o começo (a bailarina termina longe de onde começou). **Cogumelos para tocar de pé**: de 50 a 78 cm (eram 15 a 32), num anel de 1,15 a 2,1 m — a 1 m de altura e a 75 cm de quem entra, treze chapéus viravam um muro (visto na bancada); o alvo do toque subiu para o chapéu | `index` e `v6.html` | `v7.3` |
| **7.2** | (25/09): **a deusa que dança é a cópia do ser do céu** — "uma cópia daquela deusa gigante em escala menor dançando". A malha `alien-0` (a mesma do gigante a 60 m), com a pele camuflada dele (matéria 13), **vestida nos ossos da captura por proximidade**: o esqueleto do modelo (automático, do Tripo) não casa com o da captura, então cada vértice se prende aos dois ossos mais próximos no quadro de descanso, com peso pela 4ª potência do inverso da distância. A captura foi reexportada **com rotações** (três eixos por osso → quaternião na base da obra; conferido: 1 mm de erro). Quatro lições que custaram a noite: **as proporções** — o esqueleto tem as da bailarina (quadril a 60 %, ombros no pescoço) e o modelo as dele (quadril a 50 %); presa por proximidade, a coxa pegava a bacia e tudo torcia ("parece que ela está toda torta"). O modelo é deformado na vertical por trechos até os marcos dele (joelho, virilha, quadril, ombro, queixo, topo) caírem nas juntas, e cada região (braço/perna/tronco, por lado) só pega os ossos dela; o esqueleto de descanso tem de vir para **debaixo do modelo** antes de prender (a bailarina andou; sem isso a pele explodia); a **frente se lê pelos quadris**, não pelos ombros (as clavículas Mixamo coincidem); o quadro de descanso é o **mais parecido com a pose de repouso** (em pé, braços pendendo), escolhido pelo conversor. 25 mil vértices por quadro na CPU: 0,6 ms. `raizes.dancaProva()` confere ossos e centro | `index` e `v6.html` | `v7.2` |
...
- cada rodada grande ganha **número de versão novo** e uma marca (`git tag`) — a próxima é a **9.0**;
