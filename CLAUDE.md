...
| `DEUSA_QUE_DANCA` | `true` | **a bailarina da captura de movimento** (7.1), na floresta do começo: silhueta de vidro de água com coração de luz, 1,2 m, a 2,3 m à frente-direita. Os dados vêm de `assets/mocap/deusa-danca.json` (exportado do Unreal por `fontes/exportar_mocap_unreal.py` + `fontes/mocap_para_obra.py`); sem o arquivo, ela não existe. Posição e altura em `DANCA_AZ/DIST/PE/ALTURA` |
